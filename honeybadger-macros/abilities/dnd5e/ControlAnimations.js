/* Helper dialog to quickly roll attacks and resulting damage from
 * varying sized animted objects
 * MIT License (c) 2022 Matthew Haentschke
 * quickDialog function provided by Kekilla.
 */
...
(async () => {
  ...
  for(let attack = 0; attack < num; attack++){
      let roll = await new Roll(`${numd20}d20${modstring}+${toHit}`).evaluate({async:true})
  ...
  for( let crit = 0; crit < numCrits; crit++){
    let roll = await new Roll(critical).evaluate({async: true})
      await roll.toMessage({flavor: `Critical hit ${crit+1}, ${size} object`});
  }
  
  for( let dam = 0; dam < numHits; dam++){
    let roll = await new Roll(damage).evaluate({async:true})
