let message = game.messages
    .filter(message => message.content.includes(`dnd5e chat-card item-card`))
    .pop();

//this will get you the last message that was a dnd5e chat-card

//to do : find a way to parse the string for actor_id/item_id
let actor_id = /data-actor-id="(.*?)"/g.exec(message.content)[1];
let item_id = /data-item-id="(.*?)"/g.exec(message.content)[1];
