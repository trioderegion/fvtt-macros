...
function itemUpdateGenerator(castingLevel, summonerActor, summonedToken){
    ...
    const whisperingAura = summonedToken.actor.items.getName("Whispering Aura (Star Spawn Only)");
    itemUpdates.push({
        "_id": whisperingAura.id, 
        "data.save": {ability:"wis", dc:summonerDc, scaling:"flat"},
        "data.damage.parts": [[`2d6`,"psychic"]]
    });
    
    /** scale eye ray (negate own spell attack bonuses) */
    const eyeRay = summonedToken.actor.items.getName("Eye Ray (Beholderkin Only)");
    itemUpdates.push({
        "_id": eyeRay.id,
    ...
    const claws = summonedToken.actor.items.getName("Claws (Slaad Only)");
    itemUpdates.push({
        "_id": claws.id,
        "data.attackBonus": `- @mod - @prof + ${summonerSpellAttackMod}`,
    ...
    const psychicSlam = summonedToken.actor.items.getName("Psychic Slam (Star Spawn Only)");
    itemUpdates.push({
        "_id": psychicSlam.id,
        "data.attackBonus": `- @mod - @prof + ${summonerSpellAttackMod}`,
        "data.damage.parts": [[`1d8 + 3 + ${castingLevel}`, 'psychic']]
    ...
    /** Update multi attack with correct number of attacks */
